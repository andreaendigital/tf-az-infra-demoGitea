        ...
        stage('Terraform Init') {
            steps {
                ...
                withCredentials([
                    ...
                    string(credentialsId: 'aws-vpn-tunnel-ip', variable: 'TF_VAR_aws_vpn_gateway_ip')
                ]) {
                    dir("${TF_DIR}") {
                        sh '''
                ...
                withCredentials([
                    ...
                    dir("${TF_DIR}") {
                        script {
                            ...
                            // MySQL public IP only in replica-only mode
                            if (params.DEPLOYMENT_MODE == 'replica-only') {
                            ...
                            if (params.DEPLOYMENT_MODE == 'replica-only') {
                                echo "MySQL VM Public IP (temp): ${env.MYSQL_VM_PUBLIC_IP}"
                                echo "Note: Public IP for Ansible setup, can be removed after configuration"
                            } else {
...
"""
                        ...
                        // replica-only: only MySQL with replication from AWS RDS
                        inventoryContent = """# Ansible Inventory for Azure Gitea
# Auto-generated by Jenkins Pipeline - Mode: ${params.DEPLOYMENT_MODE}
# Generated: ${new Date()}
# Note: MySQL VM has temporary public IP for Ansible setup
...
deployment_mode=${params.DEPLOYMENT_MODE}
# Note: mysql_dbname, mysql_username, mysql_password passed via --extra-vars from Jenkins secrets
# Note: aws_rds_endpoint, aws_replication_user, aws_replication_password passed via --extra-vars
"""
                    }
        ...
        stage('Wait for MySQL VM to be Ready (Replica-Only)') {
            when {
                allOf {
                    expression { return params.DEPLOY_ANSIBLE }
                    expression { return params.DEPLOYMENT_MODE == 'replica-only' }
            ...
            steps {
                echo '⏳ Waiting for MySQL VM to be fully ready (replica-only mode)...'
                script {
                    echo "MySQL VM Public IP (temp): ${env.MYSQL_VM_PUBLIC_IP}"
                    echo "MySQL VM Private IP: ${env.MYSQL_VM_PRIVATE_IP}"
                    ...
                    retry(5) {
                        ...
                        sshagent(credentials: ['azure-ssh-key']) {
                            sh """
                                ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
                                    azureuser@${env.MYSQL_VM_PUBLIC_IP} 'echo "MySQL VM is ready"'
                            """
                        }
                    }
                }
                echo '✅ MySQL VM is ready and accessible'
            }
        }
